 ...
 */
enum struct KortexControlMode { NO_MODE = -1, POSITION = 0, VELOCITY = 1, EFFORT = 2 };

class KortexCommandHandle : public hardware_interface::JointHandle {
 public:
   ...
   */
  KortexCommandHandle(const JointStateHandle& joint_state_handle, double* pos, double* vel,
  ...
  void setCommand(double& cmd) {
    if (*mode_ == KortexControlMode::EFFORT)
      *eff_ = cmd;
    else if (*mode_ == KortexControlMode::EFFORT)
      *pos_ = cmd;
    else if (*mode_ == KortexControlMode::VELOCITY)
      *vel_ = cmd;
  }
...
class KortexCommandInterface
    : public hardware_interface::HardwareResourceManager<KortexCommandHandle,
